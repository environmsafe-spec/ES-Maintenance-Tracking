...
**Invoice dates default to today but are always editable.** A new invoice opens with `date: todayStr()` as a *starting point* — the field is an ordinary editable `<input type="date">`, never read-only, and whatever the user types replaces it. It is still marked required and the save is refused if it is cleared (`inv_need_date`). Opening an old invoice for editing shows the date it was saved with (`inv.date || ''`) — never today, so an existing record is not silently re-dated. Due date and payment date are NOT defaulted: `dueDate` starts empty, and Record payment prompts for `paidDate` with today offered only as editable prompt text. `todayStr()` also drives the Reports-tab period defaults and the `dueDate` overdue comparison.
