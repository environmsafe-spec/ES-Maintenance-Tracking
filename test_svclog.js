...
(async()=>{
  ...
  check('generator column present (10 base + 1)', rt.headers.length===11, 'got '+rt.headers.length);
  check('severity flagged at shifted index', rt.bad.some(r=>r.indexOf(true)>=0), JSON.stringify(rt.bad));
  run(sb,`STATE.reportGenId='g1';`);
  rt = run(sb,'buildReportTable()');
  check('single-gen fault report has no generator column (10)', rt.headers.length===10, 'got '+rt.headers.length);
